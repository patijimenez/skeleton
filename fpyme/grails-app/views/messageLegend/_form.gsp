...
<div class="fieldcontain ${hasErrors(bean: messageLegendInstance, field: 'contextCall', 'error')} ">
	...
	<g:select id="contextCall" name="contextCall.id" from="${mx.gob.inadem.Call.list()}" optionKey="id" value="${messageLegendInstance?.contextCall?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>
